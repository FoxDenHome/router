# jan/20/2023 11:56:45 by RouterOS 7.7
# software id = REMOVED
#
# model = CRS305-1G-4S+
# serial number = REMOVED
/interface bridge add admin-mac=18:FD:74:7B:0C:B9 auto-mac=no name=bridge priority=0xA000 vlan-filtering=yes
/interface ethernet set [ find default-name=ether1 ] advertise=100M-full,1000M-full l2mtu=1514 name=oob rx-flow-control=on tx-flow-control=on
...
/interface ethernet set [ find default-name=sfp-sfpplus2 ] advertise=100M-full,1000M-full,10000M-full,2500M-full,5000M-full l2mtu=9092 mtu=9000 name=sfp2-ups-dori-office rx-flow-control=on speed=10Gbps tx-flow-control=on
...
/ip dhcp-server add address-pool=pool-oob interface=oob name=dhcp-oob
...
/interface bridge port add bridge=bridge frame-types=admit-only-untagged-and-priority-tagged interface=sfp2-ups-dori-office multicast-router=disabled
...
/tool sniffer set filter-interface=oob filter-mac-protocol=lldp
